...
def identify_consensus(f_range, r_idx, col_idx, buffer_dir, all_files, allowed_tiers, res):
    ...
    for f in top_frames:
        ...
        if len(scores) > 1:
            ...
            if z_score > 1.0: # Base minimum uniqueness to count
    ...
    # 5. Last Resort Fallback (If player was present but below threshold, or just noisy slot)
...
def process_floor_tier(floor_data, dna_map, buffer_dir, all_files, res):
    ...
    if hasattr(cfg, 'BOSS_DATA') and f_id in cfg.BOSS_DATA:
        ...
        for s_idx in range(24):
            ...
            results[f"R{r+1}_S{c}_z"], results[f"R{r+1}_S{c}_mom"], results[f"R{r+1}_S{c}_pmax"] = 0.0, 0, 0.0
        return results
...
def run_tier_identification():
    print(f"--- STEP 4.2: TIER IDENTIFICATION v3.6 (Probabilistic Physical Anchor) ---")
    ...
    for _, row in final_df.iterrows():
        ...
        for r_idx in range(4):
            for col in range(6):
                ...
                # Safeguard against NaN in diagnostic output
