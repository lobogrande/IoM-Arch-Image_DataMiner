...
def get_slot_scores(roi_gray, mask, templates):
    ...
    ore_matches = [cv2.matchTemplate(roi_gray, ore, cv2.TM_CCORR_NORMED, mask=mask).max() for ore in templates['active']]
    ore_max = max(ore_matches) if ore_matches else 0
    ...
    for start, end in WINDOWS:
        ...
        for i in range(start, end + 1):
            ...
            for c in range(6): 
                ...
                bg, ore = get_slot_scores(roi, txt_mask if c in [2,3] else std_mask, templates)
                delta = ore - bg
                deltas.append(f"{delta:+.3f}")
                if delta > 0: is_pristine = False # The "All-or-Nothing" Rule
            
            p_status = "YES" if is_pristine else "NO (Ore/Shadow detected)"
