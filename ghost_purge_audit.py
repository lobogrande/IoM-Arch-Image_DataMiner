...
OUTPUT_DIR = f"diagnostic_results/Issue1_V37_{TIMESTAMP}"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# GATES (Refined for relative competition)
D_GATE = 6
O_GATE = 0.68
# The 'Golden Delta' - Ores must beat background texture by this much
MIN_DELTA_OVER_BG = 0.035 

def run_issue1_v37_audit():
    # 1. Load Assets
    ...
    print(f"--- Running Issue 1 Audit v3.7 (Relative Competition) ---")
    ...
    for f_num in PROBLEM_FLOORS:
        ...
        for slot in range(24):
            ...
            if diff <= D_GATE: continue

            # --- GATE 2: MULTI-WAY COMPETITION ---
            # Mask ignores the top text zone for identification accuracy
            ...
            # Background Match (Full ROI to capture texture comparison)
            ...
            # --- DECISION TREE ---
            is_ore = (best_o > O_GATE) and (best_o - bg_match > MIN_DELTA_OVER_BG)
            
            # If it's the top row, we must be sure it's not JUST text.
            if slot < 6:
                # If Ore is weak OR UI Text is significantly stronger than the Ore, it's Cyan
                if not is_ore or (best_u > best_o + 0.05) or (best_o < 0.82 and np.max(roi[5:15, :]) > 242):
        ...
        cv2.imwrite(os.path.join(OUTPUT_DIR, f"V37_F{f_num}.jpg"), raw_img)

if __name__ == "__main__":
    run_issue1_v37_audit()