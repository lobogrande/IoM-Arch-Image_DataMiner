...
BOSS_DATA = {
    ...
    34: {'tier': 'mixed', 'special': {8: 'myth1', 9: 'myth1', 14: 'myth1', 15: 'myth1'}}, # Simplified for template loading
...
def run_v5_27_rev1():
    ...
    print(f"--- Running v5.27-REV1: Stabilized with Original Data ---")
    ...
    for i in range(1, len(files)):
        ...
        if hud_diff > 5.0 or not is_p_in_0:
            # PRUNING: Using your exact provided mappings
            active_list = []
            allowed_tiers = [tier for tier, (low, high) in ORE_RESTRICTIONS.items() if low <= confirmed_count <= high]
            
            # Special logic for Boss Floors
            if confirmed_count in BOSS_DATA:
                boss_tier = BOSS_DATA[confirmed_count]['tier']
                if boss_tier != 'mixed' and boss_tier not in allowed_tiers:
                    allowed_tiers.append(boss_tier)
            ...
            if row1_clean and not is_p_in_0 and (this_dna != anchor['dna'] or i > anchor['idx'] + 25):
                ...
                print(f" >>> [VACUUM] F{confirmed_count} at {i:05} | DNA: {format_dna(this_dna)}")
                anchor = {"num": confirmed_count, "idx": i, "dna": this_dna, "hud": cur_hud.copy()}
                continue

            if hud_diff > 5.0 and this_dna != anchor['dna']:
                ...
                if (ml[1] + 150) <= ROW_0_MAX_Y:
                    ...
                    print(f" >>> [STRIKE] F{confirmed_count} at {i:05} | DNA: {format_dna(this_dna)}")
                    anchor = {"num": confirmed_count, "idx": i, "dna": this_dna, "hud": cur_hud.copy()}
