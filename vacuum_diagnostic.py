...
# Expanded Zone for Player Search
...
def is_player_present(full_gray, p_right, p_left):
    # 1. Damage Number Filtering
    # Crop the player lane and remove pure white 'damage' pixels
    roi = full_gray[PLAYER_ROI_Y[0]:PLAYER_ROI_Y[1], 0:350].copy()
    roi[roi > 210] = 128 # Neutralize bright damage text noise
    
    # 2. Check at 100% and 95% Scale (for recoil compression)
    best_score = 0
    for p_temp in [p_right, p_left]:
        # Standard match
        s1 = cv2.minMaxLoc(cv2.matchTemplate(roi, p_temp, cv2.TM_CCOEFF_NORMED))[1]
        # Recoil match (slightly smaller)
    ...
    return best_score > 0.65 # Lowered threshold due to animation motion blur
...
def run_v4_55_noise_filtered_auditor():
    buffer_root, out_dir = "capture_buffer_0", "diag_v4_55_FILTERED"
    ...
    print(f"--- Running v4.55: Noise-Filtered Auditor [{SCAN_START}-{SCAN_END}] ---")

    for i in range(SCAN_START, min(SCAN_END, len(files))):
        img_gray = cv2.imread(os.path.join(buffer_root, files[i]), 0)
        
        # 1. FILTERED PLAYER GATE
        ...
        if this_dna != last_dna or (i > last_idx + 25):
            floor_count += 1
            last_dna = this_dna
            print(f" >>> [HIT] Idx {i} | DNA: {'|'.join([this_dna[k:k+6] for k in range(0,24,6)])}")
        
        last_idx = i
        cv2.imwrite(os.path.join(out_dir, f"Idx{i:05}.jpg"), img_bgr_full)

    print(f"\n[FINISH] Audit complete in {time.time()-start_time:.1f}s.")

if __name__ == "__main__": run_v4_55_noise_filtered_auditor()