...
SURGICAL_OUT = "Surgical_Proof_Steel"

def run_steel_discovery():
    ...
    for zone in TEST_ZONES:
        ...
        for i in range(start_idx + 1, min(zone['end_idx'], len(frames) - 8)):
            # 1. TEMPORAL GATE: Prevents multi-frame flickering spikes
            if i < last_found_idx + 12:
                continue
            ...
            # 2. THE STEEL TRIGGER: Dual-ROI Sync Pulse
            h_flux = np.mean(cv2.absdiff(curr_h, prev_h))
            c_flux = np.mean(cv2.absdiff(curr_c, prev_c))
            
            if h_flux > 1.6 and c_flux > 1.1:
                # 3. NOISE PERSISTENCE: 5-frame future lookback
                is_permanent = True
                for offset in range(1, 5):
                    future_h = cv2.imread(os.path.join(buffer_path, frames[i+offset]), 0)[52:76, 100:142]
                    if np.mean(cv2.absdiff(future_h, curr_h)) > 6.0: # Banner velocity check
                        is_permanent = False; break
                
                # 4. IDENTITY VETO: Mandatory Structural Change
                res = cv2.matchTemplate(curr_h, anchor_h, cv2.TM_CCORR_NORMED)
                identity = res.max()
                anc_diff = np.mean(cv2.absdiff(curr_h, anchor_h))
                
                # Steel Gate: Needs low identity AND significant literal shift
                if is_permanent and identity < 0.975 and anc_diff > 2.6:
                    found_count += 1
                    last_found_idx = i
                    anchor_idx = i + 4 # Optimized anchor point
                    
                    anchor_h = cv2.imread(os.path.join(buffer_path, frames[anchor_idx]), 0)[52:76, 100:142]
                    print(f" [+] STEEL TR_{found_count} @ Idx {i} (Identity: {identity:.4f}, AncDiff: {anc_diff:.2f})")
