...
class Player:
    ...
    def _excel_floor(self, val, decimals):
        """Replicates Excel's FLOOR by bypassing Python's floating-point precision binary errors."""
        mult = 10 ** decimals
        return math.floor((val + 1e-9) * mult) / mult

    def _excel_round(self, val, decimals):
        """Replicates Excel's ROUND (Half-Up) rather than Python's default Banker's Rounding."""
        mult = 10 ** decimals
    ...
    def armor_pen(self):
        ...
        return self._excel_round(base_ap * mult_ap, 0)
    ...
    # ==========================================================================
    def print_debug_equations(self):
        print("\n" + "="*50)
        print(" 🔍 DEBUG EQUATION TRACES 🔍")
        print("="*50)
        
        # Max Stamina Trace
        f14, f23, h39, f3 = self.u('F14'), self.u('F23'), self.u('H39'), self.u('F3')
        stat_agi, f26 = self.stat('Agi'), self.u('F26')
        h28, f54, corr = self.u('H28'), self.u('F54'), self.stat('Corr')
        
        base = 100 + f14 + f23 + h39 + f3
        agi_comp = stat_agi * (5 + f26)
        asc2_comp = (1 + h28 + f54) * (1 - 0.03 * corr)
        floor_comp = 1 + (0.01 * min(100, self.current_max_floor))
        ...
        print(f"  Floor Comp:                1 + 0.01*{min(100, self.current_max_floor)} = {floor_comp:.2f}")
    ...
    def generate_json_template(self, filepath):
        ...
        with open(filepath, 'w') as f: json.dump(template, f, indent=4)
    ...
    def load_state_from_json(self, filepath):
        ...
        self.arch_ability_infernal_bonus = s.get('arch_ability_infernal_bonus', 0.0)
