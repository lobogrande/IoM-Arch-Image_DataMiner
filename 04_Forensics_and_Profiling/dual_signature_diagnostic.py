...
def run_v35_audit():
    ...
    for f in os.listdir(cfg.TEMPLATE_DIR):
        ...
        if img is not None: all_block_t.append({'name': f.split("_")[0], 'img': cv2.resize(img, (48, 48))})
    ...
    for f_num in TARGET_FLOORS:
        ...
        valid_block_t = [t for t in all_block_t if cfg.ORE_RESTRICTIONS.get(t['name'].lower(), (0,999))[0] <= f_num <= cfg.ORE_RESTRICTIONS.get(t['name'].lower(), (0,999))[1]]
        ...
        for slot in range(24):
            ...
            if not is_boss and (best_o < 0.78 or (np.max(roi) > 235 and best_o < 0.90)):
                t_score, t_label = get_temporal_identity((x1, y1, x1+48, y1+48), valid_block_t, f_name, mask)
