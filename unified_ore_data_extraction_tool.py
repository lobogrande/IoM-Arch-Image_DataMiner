...
MINING_OUT = "Production_Mining_Results_v22"
...
def load_all_templates():
    ...
    for f in os.listdir(t_path):
        ...
        if img.shape != (48, 48): img = cv2.resize(img, (48, 48))
...
def run_accuracy_miner():
    ...
    for run_dir in runs:
        ...
        # TUNING: Run 3 requires higher sensitivity due to Audit findings
        base_threshold = 0.74 if run_id == "3" else 0.77
        
        with open(os.path.join(run_path, "final_sequence.json"), 'r') as f: sequence = json.load(f)
                    ...
                    for tier in check_list:
                        ...
                        for state in ['act', 'sha']:
                            for t_img in templates['ore'][tier][state]:
                                ...
                                score = res.max()
                                # SHADOW BOOST: Give shadows a 3% boost to clear background noise
                                if state == 'sha': score *= 1.03
                                if score > best_ore['score']:
                                    best_ore = {'tier': tier, 'score': score, 'state': state}

                # V2.2 Tuning: Differential Gate lowered to 0.02 for high-noise deep floors
                delta = best_ore['score'] - best_bg_score
                if best_ore['score'] > base_threshold and delta > 0.02:
