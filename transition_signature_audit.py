...
def get_slot_status_worker(args):
    ...
    for t_list in templates['active']:
        ...
        if s > ore_s: ore_s = s
    delta = ore_s - bg_s
    # Use v5.27 stabilized thresholds
...
def run_transition_signature_audit():
    ...
    print(f"--- Real Transition Signature Audit (Indices {START_IDX}-{END_IDX}) ---")
    ...
    print(f"{'Idx':<6} | {'HUD D.':<7} | {'Hamming':<7} | {'Row 1':<8} | {'Status'}")
    print("-" * 60)
    ...
    for i in range(START_IDX, END_IDX):
        ...
        curr_dna = get_dna_at(i, 1) # Static floor 1 check for signature
        hamming = get_hamming_distance(anchor_dna, curr_dna)
        row1 = curr_dna[:6]

        note = ""
        # Highlight known transition areas (e.g., F2 call was at 00043)
        if i == 43: note = "<< F2 CALLED"
        if i == 63: note = "<< F3 SHOULD BE HERE"

        if hamming > 0 or hud_diff > 1.0:
            print(f"{i:<6} | {hud_diff:<7.2f} | {hamming:<7} | {row1:<8} | {note}")
