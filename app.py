...
import project_config as cfg # <--- Import your configurations
    ...
    for upg_id, upg_data in p.UPGRADE_DEF.items():
        ...
        with current_col:
            ...
            with st.container(border=True):
                ...
                # Fetch cap from config (fallback to 99 if missing)
                max_lvl = cfg.INTERNAL_UPGRADE_CAPS.get(upg_id, 99)
                
                # Clean UI Text with Max Value
                st.markdown(f"**[{upg_id}] {name}** (Max: {max_lvl})")
                
                current_lvl = p.upgrade_levels.get(upg_id, 0)
                
                # Safe fallback: If a user uploads a broken JSON with Level 60 but max is 50, 
                # clamp it to 50 so Streamlit doesn't crash throwing a ValueError.
                safe_val = min(current_lvl, max_lvl)
