...
def is_slot_clean_v25(img_gray, slot_idx, row_idx, templates, is_sliver=False, side='left'):
    ...
    # Use 0.77 for sliver (high precision) and 0.70 for full tile (wide net)
...
def run_v25_production():
    buffer_root = "capture_buffer_0"
    out_dir = "production_audit_v25"
    ...
    anchor = {
        ...
        "hud": f0_gray[HEADER_ROI[0]:HEADER_ROI[1], HEADER_ROI[2]:HEADER_ROI[3]].copy(), 
        "pos": (0, 0), "ore": "pending"
    }
    confirmed = []

    print(f"--- Running v25.0: Multi-Stage Path Auditor (Cap: {TARGET_FLOOR_LIMIT}) ---")
    ...
    for i in range(1, len(files)):
        ...
        # 1. GLOBAL SCAN (Row 1 then Row 2)
        ...
        if (i - anchor['idx'] > 5) and (mae > 3.5):
            ...
            # 3. Path Audit (The Multi-Stage Fix)
            path_is_clean = True
            if mode == 'ROW1':
                if n_slot > 0 and not is_slot_clean_v25(img_gray, n_slot-1, 0, ore_tpls, True, 'left'): path_is_clean = False
                if path_is_clean and n_slot >= 2:
                    for s in range(n_slot-1): 
                        if not is_slot_clean_v25(img_gray, s, 0, ore_tpls, False): path_is_clean = False; break
            else: # ROW2
                if n_slot < 5 and not is_slot_clean_v25(img_gray, n_slot+1, 1, ore_tpls, True, 'right'): path_is_clean = False
                if path_is_clean and n_slot <= 3:
                    for s in range(n_slot+2, 6):
                        if not is_slot_clean_v25(img_gray, s, 1, ore_tpls, False): path_is_clean = False; break
            ...
            if path_is_clean:
                ...
                print(f" [OK] F{f_num} -> F{target_f:=target_f:=f_num+1} ({mode}) at Index {i}")

    cv2.imwrite(f"{out_dir}/confirmed/F{anchor['num']:03}_Idx{anchor['idx']:05}.jpg", anchor['img'])
    print(f"\n[FINISH] Scanned {i} frames. Total confirmed: {len(confirmed)+1}")
