...
def get_ore_id_v18_4(img_gray, slot_idx, current_floor, templates):
...
def run_v18_4_gravity_aware():
    buffer_root = "capture_buffer_0"
    out_dir = "production_audit_v18_4"
    ...
    anchor = {
        "num": 1, "idx": 0, "pos": (max_loc_init[0], max_loc_init[1] + 150), "slot": init_slot,
        "img": f1_bgr.copy(),
        "hud": f1_gray[HEADER_ROI[0]:HEADER_ROI[1], HEADER_ROI[2]:HEADER_ROI[3]],
        "ore": get_ore_id_v18_4(f1_gray, init_slot, 1, ore_tpls),
        "dna": get_grid_dna_fast(f1_gray)
    }
    confirmed = []

    print("--- Running v18.4: Gravity-Aware Top-Down Auditor ---")

    for i in range(1, len(files)):
        img_bgr = cv2.imread(os.path.join(buffer_root, files[i]))
        img_gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
        
        # 1. Full-Search for Player (Accuracy over speed)
        ...
        # 3. SOFT-RANKING TRIGGER
        # We commit if there's a strong reason, but we check if the player is in a "valid" height
        commit_eligible = (mae > 3.5) or (dist > 50 and dna_diff >= 4)
        
        if commit_eligible and (i - anchor['idx'] > 2):
            # Prioritize frames where player is HIGH (Row 0-1)
            # If the player is way down in Row 3 (Y > 400), we flag it for debug
            is_valid_spawn = (actual_y < 350) 
            ...
            if is_valid_spawn:
                ...
                anchor = {
                    ...
                    "ore": get_ore_id_v18_4(img_gray, slot_x, target_f, ore_tpls),
                    "dna": cur_dna
                }
                print(f" [OK] Floor {f_num} confirmed at Index {i}")
            else:
                # If we're losing floors, this debug file will tell us if it's because the player is too low
                cv2.imwrite(f"{out_dir}/debug/Idx{i:05}_LOW_PLAYER_Y{actual_y}.jpg", img_bgr)
