...
OUTPUT_DIR = f"diagnostic_results/FloorDNA_v76_{datetime.now().strftime('%H%M')}"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# GATES
D_GATE = 7.5       
FLOOR_SWAP_MIN = 8 
CHANGE_SENSITIVITY = 10.0 
...
def run_v76_live_audit():
    ...
    print(f"--- Running v7.6 High-Speed Audit (Run_{TARGET_RUN}) ---")
    print(f" [+] Found {len(buffer_files)} frames. Commencing scan...")

    for i, fname in enumerate(buffer_files):
        if i % 100 == 0 and i > 0:
            elapsed = time.time() - start_time
            fps = i / elapsed if elapsed > 0 else 0
            print(f"  > Processing: {i}/{len(buffer_files)} frames... ({fps:.1f} FPS)")
        ...
        if raw_diff > CHANGE_SENSITIVITY:
            ...
            if diff_count >= FLOOR_SWAP_MIN:
                ...
                current_start_idx = i
                print(f" [!] Boundary Found: Floor {floor_num} -> {floor_num+1} (Frame {i})")
            
            elif diff_count > 0:
                active_vector = new_vector 

        last_frame_gray = curr_gray # THE FIX: This now updates correctly

    print(f"\n[SUCCESS] Mapped {len(floor_library)} floors in {time.time()-start_time:.1f}s.")
