...
OUTPUT_DIR = f"diagnostic_results/FloorDNA_v25_StateAware_{datetime.now().strftime('%m%d_%H%M')}"
...
def get_lower_surgical_vector(img_gray, bg_templates):
    ...
    for slot in range(24):
        ...
        # Samples BOTTOM quadrant to avoid damage number noise
...
def run_v25_state_aware_audit():
    ...
    print(f"--- Running v25.0 State-Aware Consensus Engine ---")
    ...
    for i in range(len(buffer_files) - 3):
        ...
        # Tiered Refractory (v16/v23 Baseline)
        ...
        total_delta = spawns + despawns
        count_n, count_n1 = sum(vec_n), sum(vec_n1)

        is_new_floor = False
        if frames_since_last_call > ref_limit:
            
            # PROFILE 1: THE STEP-DOWN (Boss Clear Fix)
            if count_n >= 22 and count_n1 <= 19 and hud_pulse > HUD_PULSE_THRES:
                is_new_floor = True
            
            # PROFILE 2: STANDARD GENERATION (N1 must persist in N2)
            elif spawns >= 2 and vec_n1 == vec_n2:
                # Add row-entropy for mid/late stages
                rows = set([divmod(s, 6)[0] for s in [j for j in range(24) if vec_n[j] == 0 and vec_n1[j] == 1]])
                if curr_floor_count < 20 or len(rows) > 1 or spawns >= 5:
                    is_new_floor = True
            
            # PROFILE 3: MASSIVE WIPE (Quake Catch-all)
            elif total_delta >= 15:
                is_new_floor = True
        ...
        if is_new_floor:
            ...
            print(f" [!] Boundary {floor_num} Logged (Profile Match)")
