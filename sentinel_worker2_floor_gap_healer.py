...
GRID_CENTER = (280, 200) # Point to watch for floor-clear flashes

def run_forensic_healer():
    ...
    for ds_id in DATASETS:
        ...
        # 1. CEILING FINDER: Scan for floors after the last anchor
        last_anc = anchors[-1]
        ceiling_f = find_ceiling(buffer_path, frames, last_anc, digit_map)
        if ceiling_f and ceiling_f['floor'] > last_anc['floor']:
            anchors.append(ceiling_f)
            print(f" Ceiling Found: Floor {ceiling_f['floor']} at Frame {ceiling_f['idx']}")

        all_floors = []
        # 2. GAP HEALING LOOP
        for i in range(len(anchors) - 1):
            all_floors.append(anchors[i])
            start_f, end_f = anchors[i]['floor'], anchors[i+1]['floor']
            start_idx, end_idx = anchors[i]['idx'], anchors[i+1]['idx']
            
            if (end_f - start_f) > 1:
                print(f" Healing Gap: F{start_f} to F{end_f} ({end_f - start_f - 1} missing)")
                # Targeted Scan inside this frame window
                found = targeted_gap_scan(buffer_path, frames, start_idx, end_idx, start_f, end_f, digit_map)
                all_floors.extend(found)
                for f in found:
                    save_healed_evidence(buffer_path, f, heal_path)

        all_floors.append(anchors[-1])
        with open(f"healed_consensus_run_{ds_id}.json", 'w') as f:
            json.dump(all_floors, f, indent=4)

def find_ceiling(path, frames, last_anc, digit_map):
    best_f = None
    for i in range(len(frames) - 1, last_anc['idx'], -1):
        ...
        val = get_bitwise_number(roi, digit_map, 175, 0.75) # Lower threshold for ceiling discovery
        if val > last_anc['floor']:
            return {'idx': i, 'floor': val, 'frame': frames[i]}
    return None

def targeted_gap_scan(path, frames, start_idx, end_idx, start_f, end_f, digit_map):
    healed = []
    expected_count = end_f - start_f - 1
    current_target = start_f + 1
    
    # We look for the FIRST stable instance of each missing floor number
    for i in range(start_idx + 1, end_idx):
        gray = cv2.imread(os.path.join(path, frames[i]), 0)
        roi = gray[HEADER_ROI[0]:HEADER_ROI[1], HEADER_ROI[2]:HEADER_ROI[3]]
        val = get_bitwise_number(roi, digit_map, 165, 0.70) # High sensitivity for healing
        
        if val == current_target:
            healed.append({'idx': i, 'floor': val, 'frame': frames[i]})
            current_target += 1
            if len(healed) == expected_count: break
...
def save_healed_evidence(path, milestone, heal_path):
    ...
    # HUD restored for Worker 2 verification
    cv2.rectangle(img, (103, 54), (138, 74), (0, 255, 0), 2)
    cv2.putText(img, "HEALED", (103, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
