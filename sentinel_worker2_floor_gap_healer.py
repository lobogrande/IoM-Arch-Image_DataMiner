...
GRID_ROI = (250, 600, 50, 450) # ROI for pixel delta checking

def run_obsessive_healer():
    ...
    for ds_id in DATASETS:
        ...
        print(f"\n--- OBSESSIVE HEALING RUN {ds_id} ---")
        
        # 1. CEILING FINDER
        last_anc = anchors[-1]
        ceiling_f = find_ceiling_bruteforce(buffer_path, frames, last_anc, digit_map)
        ...
        for i in range(len(anchors) - 1):
            ...
            if (end_f - start_f) > 1:
                print(f" Closing Gap: F{start_f} to F{end_f} ({end_f - start_f - 1} floors)")
                found = recursive_siege(buffer_path, frames, start_idx, end_idx, start_f, end_f, digit_map)
                final_milestones.extend(found)
                for f in found: save_healed_evidence(buffer_path, f, heal_path)
...
def recursive_siege(path, frames, start_idx, end_idx, start_f, end_f, digit_map):
    healed = []
    ...
    for target in missing_floors:
        ...
        # Pass 1-3: Increasing Sensitivity
        for conf, thresholds in [(0.80, [175, 165]), (0.72, [155, 195]), (0.65, [145, 205])]:
            for t in thresholds:
                for i in range(current_search_idx, end_idx):
                    gray = cv2.imread(os.path.join(path, frames[i]), 0)
                    roi = gray[HEADER_ROI[0]:HEADER_ROI[1], HEADER_ROI[2]:HEADER_ROI[3]]
                    if get_bitwise_number(roi, digit_map, t, conf) == target:
                        found_milestone = {'idx': i, 'floor': target, 'frame': frames[i]}
                        break
                if found_milestone: break
            if found_milestone: break
            
        # Pass 4: Fallback Interpolation (Pixel Delta)
        if not found_milestone:
            print(f" !! OCR Failed for F{target}. nominating via pixel delta...")
            found_milestone = nominate_via_delta(path, frames, current_search_idx, end_idx, target)
            
        healed.append(found_milestone)
        current_search_idx = found_milestone['idx'] + 1
        
    return healed

def nominate_via_delta(path, frames, start, end, floor):
    max_delta = -1
    best_idx = start
    prev_grid = None
    
    # We find the single largest board change in this window
    for i in range(start, min(start + 500, end)): # Limit search to prevent run-away
        img = cv2.imread(os.path.join(path, frames[i]), 0)
        grid = img[GRID_ROI[0]:GRID_ROI[1], GRID_ROI[2]:GRID_ROI[3]]
        if prev_grid is not None:
            delta = np.sum(cv2.absdiff(grid, prev_grid))
            if delta > max_delta:
                max_delta = delta
                best_idx = i
        prev_grid = grid
...
def find_ceiling_bruteforce(path, frames, last_anc, digit_map):
    for i in range(len(frames) - 1, last_anc['idx'], -1):
        gray = cv2.imread(os.path.join(path, frames[i]), 0)
        val = get_bitwise_number(gray[54:74, 103:138], digit_map, 165, 0.65)
...
def save_healed_evidence(path, milestone, heal_path):
    ...
    cv2.rectangle(img, (103, 54), (138, 74), (0, 0, 255), 2)
    cv2.putText(img, "HEALED", (103, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
...
def perform_final_audit(run_id, milestones):
    ...
    missing = sorted(list(set(range(1, milestones[-1]['floor'] + 1)) - set(found)))
    print(f"--- FINAL OBSESSIVE AUDIT RUN {run_id} ---")
    print(f" Status: {'SUCCESS' if not missing else 'INCOMPLETE'}")
    print(f" Floors: 1 to {milestones[-1]['floor']} | Found: {len(milestones)}")
    if missing: print(f" Missing: {missing}")

if __name__ == "__main__":
    run_obsessive_healer()