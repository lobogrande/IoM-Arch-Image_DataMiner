    ...
    for ds_id in DATASETS:
        ...
        for i in range(len(anchors) - 1):
            ...
            if (e_f - s_f) > 1:
                ...
                sys.stdout.flush()
                healed = solve_gap_with_plateaus(buffer_path, frames, anchors[i], anchors[i+1], digit_map)
...
def solve_gap_with_plateaus(path, frames, anc_s, anc_e, digit_map):
    healed = []
    missing_floors = list(range(anc_s['floor'] + 1, anc_e['floor']))
    
    # 1. SIGNATURE EVICTION: Lock out the frames belonging to the previous anchor
    img_s = cv2.imread(os.path.join(path, anc_s['frame']), 0)
    ...
    for i in range(anc_s['idx'] + 1, anc_e['idx']):
        ...
        if np.mean(cv2.absdiff(curr, sig_s)) > 15: 
            current_search_start = i
            break
    ...
    for target in missing_floors:
        ...
        # 2. SEARCH: High-stability OCR gate
        ...
        if not found_m:
            # 3. CORRECTED FALLBACK: Find quietest moment *after* the previous floor signature breaks
            print(f"   [F{target}] Blind Fallback - Finding quiet plateau...")
            found_m = nominate_quiet_plateau(path, frames, current_search_start, anc_e['idx'], target)
            
        print(f"   [F{target}] Locked: {found_m['frame']}")
        sys.stdout.flush()
        healed.append(found_m)
        
        # 4. ITERATIVE LOCK: Find when this new floor signature ends before starting the next search
        img_new = cv2.imread(os.path.join(path, found_m['frame']), 0)
        sig_new = img_new[HEADER_ROI[0]:HEADER_ROI[1], HEADER_ROI[2]:HEADER_ROI[3]]
        for i in range(found_m['idx'] + 1, anc_e['idx']):
            curr = cv2.imread(os.path.join(path, frames[i]), 0)[HEADER_ROI[0]:HEADER_ROI[1], HEADER_ROI[2]:HEADER_ROI[3]]
            if np.mean(cv2.absdiff(curr, sig_new)) > 20: 
                current_search_start = i
                break
        else:
            current_search_start = found_m['idx'] + 5 
...
def nominate_quiet_plateau(path, frames, start, end, floor):
    ...
    best_idx = start
    limit = min(start + 250, end)
    
    # We compare 10-frame windows to find the one with the least visual variance
    for i in range(start, limit - 10):
        variances = []
        for j in range(10):
            gray = cv2.imread(os.path.join(path, frames[i+j]), 0)
            roi = gray[HEADER_ROI[0]:HEADER_ROI[1], HEADER_ROI[2]:HEADER_ROI[3]]
            variances.append(roi)
        
        diff = np.mean([np.mean(cv2.absdiff(variances[k], variances[k+1])) for k in range(9)])
...
def save_healed_evidence(path, milestone, heal_path):
    ...
    cv2.putText(img, f"PLATEAU F{milestone['floor']}", (100, 48), 0, 0.4, (0, 255, 0), 1)
